...
(function () {
    ...
    Copter.prototype.init = function(drawingState) {
        ...
        var car = LoadedOBJFiles["Samand400.obj"];
        console.log(car.vertices.length);

        var data = [];//[geometries.vertices.length*3];
        for(var i = 0; i < car.vertices.length; i++){
            for(var j = 0; j < 3; j++) {
                data[(i*3) + j] = car.vertices[i][j];
            }
        }
        ...
        for(var i = 0; i < car.vertices.length; i++){
            for(var j = 0; j < 3; j++) {
                data2[(i*3) + j] = car.vertices[i][j];
            }
        }
        for(var i = 0; i < car.normals.length; i++){
            for(var j = 0; j < 3; j++) {
                data2[(i*3) + j] = car.normals[i][j];
            }
        }
//get indices
        var data3 = [];
        var k = 0;
        for(var i = 0; i < car.groups['car' ].faces.length; i++){
            var tmp = car.groups['car' ].faces[i];
            for(var j = 0; j < 3; j++) {
                var tmp2 = tmp[j];
                data3[k] = tmp2[0];
                k++;
