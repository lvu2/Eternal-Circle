...
window.onload = function() {
    ...
    function draw() {
        ...
        if (uiMode.value == "ArcBall") {
            ...
            if (keysdown[38]) { yDir -= .5; } // down key
            if (keysdown[40]) { yDir += .5; } // up key
            if (keysdown[83]) { zDir -= .5; } // w key
            if (keysdown[87]) { zDir += .5; } // s key
            if (keysdown[68]) { xDir -= .5; } // a key
            if (keysdown[65]) { xDir += .5; } // d key
            twgl.m4.setTranslation(viewM, [xDir, yDir, zDir], viewM); // mine
